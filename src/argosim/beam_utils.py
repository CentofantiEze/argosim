...
class CosCubeBeam():
    ...
    def __init__(self, c=.2, f=1., n_pix=100, fov_deg=1.):
        self.c = c
        self.f = f
    ...
    def check_fov(self):
        ...
        if np.sqrt(2)*self.fov_deg > self.beam_edge():
            print('Warning: FOV diagonal lenght ({:.2f} deg) is larger than beam edge ({:.2f} deg).'.format(np.sqrt(2)*self.fov_deg, self.beam_edge()))
            print('Suggested FOV: {:.2f} deg'.format( np.floor(100*self.beam_edge()/np.sqrt(2))/100 ) )
    ...
    def get_mesh(self):
        ...
        l, m = np.meshgrid(np.linspace(-self.fov_deg, self.fov_deg, self.grid_size), np.linspace(-self.fov_deg, self.fov_deg, self.grid_size))
        return l, m
    ...
    def plot_beam_1d(self, freqs):
        ...
        if freqs is None:
            freqs = [self.f]
        plt.figure()
        plt.hlines(1/np.sqrt(2), -self.fov_deg, self.fov_deg, linestyle='--', color='k', alpha=0.5, label='-3dB')
        cmap = plt.get_cmap('viridis')
        for i, f in enumerate(freqs):
            self.set_f(f)
            l = np.linspace(-self.fov_deg, self.fov_deg, self.grid_size)
            plt.plot(l, self(l, 0), color=cmap(i/len(freqs)) ,label='{} GHz'.format(self.f))
    ...
    def plot_beam_2d(self):
        z = self.get_beam()
        fig, ax = plt.subplots()
        ax.imshow(z, extent=(-self.fov_deg, self.fov_deg, -self.fov_deg, self.fov_deg), vmin=0, vmax=1)
        r_fov = self.r_fov()
        fov_contour = plt.Circle((0,0), r_fov, color='gray', alpha=0.7, linestyle='--', fill=False)
        ...
        ax.set_title(r'Cosine cubed beam - FOV: {:.1f} $\deg^2$ @ {} GHz'.format(self.fov_solid_angle(r_fov), self.f))
        plt.show()
    ...
    def evaluate_beam_fit(self, freqs, norm_beam_list):
        ...
        for i, norm_beam in enumerate(norm_beam_list):
            ax[i].plot(norm_beam[: ,0], norm_beam[: ,2], label='Main lobe', color=cmap(i/len(freqs)))
            ...
            ax[i].hlines(1/np.sqrt(2), -self.beam_edge(), self.beam_edge(), linestyle='--', color='gray', alpha=0.7, label='-3dB')

            ax[i].set_ylim(0, 1)
            ax[i].set_xlim(-self.beam_edge(), self.beam_edge())
            ax[i].set_title(r'{:.1f} GHz - FOV: {:.1f} $\deg^2$'.format(self.f, self.fov_solid_angle()))
            ax[i].legend()
